...
def calculate_best_labels(image, long_caption, short_caption):  
    ...
    for label in clip_labels:
        with torch.no_grad():
            logits_per_image = (image_features @ clip_text_features_dict[label].T).cpu().numpy()[0][0]
    ...
    while True:
        ...
        for label in labels:
            ...
            with torch.no_grad():
                logits_per_image = (image_features @ combined_text_features.T).cpu().numpy()[0][0]
...
def process_image(image_path, folder_chartag, args):
    ...
    try:
        ...
        clip_caption = []
        clip_caption, final_score = calculate_best_labels(image, more_detailed_caption, wd14_caption)
        folder_chartag = build_folder_chartag(clip_caption[4], folder_chartag)
        tags_text = (
            f"{special_text}, {clip_caption[4]}\n"
            f"{special_text}, {clip_caption[3]}\n"
            f"{special_text}, {clip_caption[2]}\n"
            f"{special_text}, {clip_caption[1]}\n"
            f"{special_text}, {clip_caption[0]}"
        )
    ...
    if all_final_scores:
        ...
        for image_path, final_score in all_final_scores:
            ...
            if relative_score >= 0.8:
                accuracy_tag = "best accuracy"
            elif relative_score >= 0.6:
                accuracy_tag = "high accuracy"
            elif relative_score >= 0.4:
                accuracy_tag = "normal accuracy"
            elif relative_score >= 0.2:
                accuracy_tag = "low accuracy"
            else:
                accuracy_tag = "worst accuracy"
            ...
            if tag_file_path.exists():
                ...
                with open(tag_file_path, 'w', encoding='utf-8') as file:
                    for i, line in enumerate(lines):
                        if i == len(lines) - 1:
                            file.write(line.strip() + f', {accuracy_tag}')  
                        else:
                            file.write(line.strip() + f', {accuracy_tag}\n') 
