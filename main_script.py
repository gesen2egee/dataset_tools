...
def generate_special_text(image_path, args, features=None, chars=None):
    ...
    if chartag_from_folder and features and "solo" in features:
        return f"{concept_tag}{chartag_from_folder}", ', '.join(chartags), boorutag, artisttag
...
def calculate_best_labels(image, short_caption, long_caption, image_path): 
    ...
    def find_best_combined_text(image_features, labels, init_text=None, _iter=10):
        ...
        if i != _iter:
            improved = False
            for label in labels:
                combined_text = f"{init_text}, {label}"
                text_tensor = longclip.tokenize([combined_text]).to(device)
                with torch.no_grad():
                    text_features = clip_model.encode_text(text_tensor)
                    text_features = F.normalize(text_features, dim=-1)
                    logits_per_image = (image_features @ text_features.T).item()
                if logits_per_image > last_score:
                    last_score = logits_per_image
                    improved = True
            if improved:
                best_labels.extend(label)
        return best_labels
    ...
    clothtag, persontag, peopletag = '', '', ''
    long_labels = [label for label in long_caption.split(", ") if label.strip() and label not in long_labels]
    labels = [label for label in short_caption.split(", ") if label.strip() and label not in labels and not (contains_color(label) and args.drop_colortag)]
    ...
    if args.clothtag and ('solo' in labels or persontag == 'focus on one person'):
        clothes_labels = [label for label in labels if label.strip() and label not in clothes_labels and label in clothing_tags]
        clothtags = []
...
def process_image(image_path, folder_chartag, args):
    ...
    try:
        ...
        clip_caption = []
        clip_caption, final_score, clothtag, persontag, peopletag = calculate_best_labels(image, wd14_caption, more_detailed_caption, image_path)
        folder_chartag = build_folder_chartag(clip_caption[4], folder_chartag)
        if persontag:
            if persontag != "focus on one person":
                special_text = f"{persontag} include " + special_text 
        if keeptag:
            special_text += f", {keeptag}"
        if clothtag:
            special_text += f", with {clothtag}"
        if peopletag:     
            special_text += f", {peopletag}"
        if ratingtag:     
