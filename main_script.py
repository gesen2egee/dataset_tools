...
def calculate_best_labels(image, short_caption, long_caption, image_path): 
    ...
    long_labels = [label for label in long_caption.split(", ") if label.strip()] 
    labels = [label for label in short_caption.split(", ") if label.strip() and not (args.drop_colortag and contains_color(label))]
    ...
    for label in labels + long_labels:
    ...
    best_long_label = None
    best_score = float('-inf')
    final_labels = {}
    for label in long_labels:
        if args.use_norm:
            combined_text_features = torch.cat([text_features_dict[l] for l in selected_labels[0].split(", ") + [label]], dim=0).mean(dim=0, keepdim=True)
            combined_text_features = F.normalize(combined_text_features, dim=-1)
        else:    
            combined_text_features = torch.cat([text_features_dict[l] for l in selected_labels[0].split(", ") + [label]], dim=0).sum(dim=0, keepdim=True)
        with torch.no_grad():
            logits_per_image = (image_features @ combined_text_features.T).item()
            final_labels[label] = logits_per_image
    sorted_labels = sorted(final_labels.items(), key=lambda item: item[1], reverse=True)
    best_long_labels = [label for label, score in sorted_labels]          
        
    return selected_labels, final_score, best_long_labels, (', ').join(top_clip_labels)
...
def process_image(image_path, folder_chartag, args):
    ...
    try:
        ...
        clip_caption = []
        cliped_wd14_caption, final_score, best_long_labels, clip_caption = calculate_best_labels(image, wd14_caption, more_detailed_caption + ', ' + detailed_caption, image_path)
        folder_chartag = build_folder_chartag(clip_caption[4], folder_chartag)
        tags_text = (
            f"{special_text}, ___{cliped_wd14_caption[2]}\n"
            f"{special_text}, ___{', '.join(best_long_labels[:1])}, {cliped_wd14_caption[1]}\n"
            f"{special_text}, ___{', '.join(best_long_labels[:2])}, {cliped_wd14_caption[0]}\n"
            f"{special_text}, ___{more_detailed_caption}\n"
            f"{special_text}, ___{cliped_wd14_caption[1]}, {detailed_caption}, {clip_caption}"
        )
